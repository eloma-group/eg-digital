...
/* ───────────────────────── Variant A — Editorial restraint ─────────────────────────
   ...
   "Space is the luxury." Motion is load-only and deliberate. */
...
/* ───────────────────────── Variant B — Cinematic text-clip ─────────────────────────
   Navy stage. The showreel plays *inside* the giant word JOURNEY via an SVG mask
   (negative-typography trend). One bold idea, cinematic stillness around it. */
  ...
  return (
    <section className="jh jh-b">
      ...
      <motion.div className="jh-b-clip" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.2, ease: EASE, delay: 0.2 }}>
        ...
        <svg className="jh-b-svg" viewBox="0 0 1200 280" preserveAspectRatio="xMidYMid meet" aria-label="Journey">
          <defs>
            <mask id="jh-b-mask">
              ...
              <text x="600" y="146" textAnchor="middle" dominantBaseline="central"
                ...
                textLength="1150" lengthAdjust="spacingAndGlyphs" fill="#000">JOURNEY</text>
            </mask>
          </defs>
          <rect width="1200" height="280" fill={NAVY} mask="url(#jh-b-mask)" />
        </svg>
      </motion.div>

      <motion.p className="jh-b-sub" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE, delay: 0.5 }}>
        From a single desk to fifty shipped products — six years of websites, apps & cloud, built in Melbourne.
      </motion.p>

      <motion.div className="jh-b-bot" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.8 }}>
        <span className="jh-scrollcue jh-scrollcue--light">Scroll to explore <i /></span>
      </motion.div>
    </section>
  )
}

/* ───────────────────────── Variant C — Kinetic word rotator ─────────────────────────
   Cream stage, editorial serif headline with one word cycling through the work
   we do. Kinetic typography — restrained, on a single line. */
const ROTATE = ['building', 'designing', 'shipping', 'scaling', 'securing']
  ...
  return (
    <section className="jh jh-c">
      <motion.span className="jh-eyebrow" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.1 }}>
        <b />Our Journey · Est. 2021
      </motion.span>

      <h1 className="jh-c-h1">
        <span className="jh-clip"><motion.span className="jh-line" initial={{ y: '115%' }} animate={{ y: '0%' }} transition={{ duration: 1.1, ease: EASE, delay: 0.15 }}>Six years of</motion.span></span>
        ...
        <span className="jh-clip"><motion.span className="jh-line" initial={{ y: '115%' }} animate={{ y: '0%' }} transition={{ duration: 1.1, ease: EASE, delay: 0.3 }}>what's next.</motion.span></span>
      </h1>
      ...
      <motion.div className="jh-c-bot" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE, delay: 0.55 }}>
        <div className="jh-ledger">
          <div><span className="num">2021</span><span className="lbl">Founded</span></div>
          <div><span className="num">50+</span><span className="lbl">Products</span></div>
          <div><span className="num">6</span><span className="lbl">Years</span></div>
        </div>
    ...
    <>
      <style>{`
        ...
        .jh-ledger > div { display: flex; flex-direction: column; gap: 6px;
          padding-right: clamp(20px,2.4vw,40px); margin-right: clamp(20px,2.4vw,40px); border-right: 1px solid rgba(255,255,255,0.14); }
        .jh-ledger > div:last-child { border: none; margin: 0; padding: 0; }
        .jh-ledger .num { font-size: clamp(26px,2.8vw,42px); font-weight: 900; letter-spacing: -0.04em; line-height: 1; color: #fff; font-variant-numeric: tabular-nums; }
        .jh-ledger .lbl { font-size: clamp(10px,0.8vw,12px); font-weight: 800; letter-spacing: 1.6px; text-transform: uppercase; color: rgba(255,255,255,0.42); }
        ...
        .jh-c-rot em { display: inline-block; font-style: italic; font-weight: 500; color: ${GREEN}; }
        .jh-c-bot { display: flex; align-items: flex-end; justify-content: space-between; gap: 32px; flex-wrap: wrap; }
        .jh-c .jh-ledger > div { border-right-color: rgba(8,33,60,0.14); }
        .jh-c .jh-ledger .num { color: ${NAVY}; }
        .jh-c .jh-ledger .lbl { color: rgba(8,33,60,0.42); }

        /* ── variant switcher (temporary preview control) ── */
        .jh-switch { position: fixed; top: 80px; right: 16px; z-index: 999; display: flex; gap: 6px;
          background: rgba(8,33,60,0.9); backdrop-filter: blur(10px); padding: 6px; border-radius: 100px; border: 1px solid rgba(255,255,255,0.15); }
        .jh-switch button { border: none; cursor: pointer; font-family: inherit; font-size: 11px; font-weight: 800;
          letter-spacing: 0.5px; padding: 8px 14px; border-radius: 100px; background: transparent; color: rgba(255,255,255,0.6); transition: all 0.18s; }
        .jh-switch button.on { background: ${GREEN}; color: ${NAVY}; }
      ...
      <div className="jh-switch">
        <button className={v === 'A' ? 'on' : ''} onClick={() => setV('A')}>Editorial</button>
        <button className={v === 'B' ? 'on' : ''} onClick={() => setV('B')}>Cinematic</button>
        <button className={v === 'C' ? 'on' : ''} onClick={() => setV('C')}>Kinetic</button>
      </div>
